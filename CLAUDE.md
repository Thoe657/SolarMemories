...
- When `graphify-out/graph.json` exists, start codebase questions with `graphify query`,
  `graphify explain`, or `graphify path`, then use the result to target source reads.
  Read `GRAPH_REPORT.md` only for broad architecture work or when queries are insufficient.
- After code changes, run `graphify update .`; the post-commit hook covers only commits
  from the main checkout. Update manually after material documentation changes.
- `.graphifyignore` is the source of truth for corpus exclusions. On Windows, keep
  `graphify-out/.graphify_root` and `.graphify_python` UTF-8 without a BOM.
