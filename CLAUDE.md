...
- `scene.js` caches each memory's generated `CanvasTexture` in an in-memory
  ...
  planet skips regenerating unchanged cards' textures. Editing a memory (`memoryForm.js`'s
  edit mode) evicts its cache entry via `updateMemoryInScene()`, so a cache hit always
  reflects the memory's current saved content.
...
Two plans are complete and merged to `main`, condensed in
